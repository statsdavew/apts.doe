...
dopt1 <- optFederov(~ quad(x), data = c(x, x, x), nTrials = 9, criterion = "D")
dopt1
```
...
$D$-optimal design to estimate a quadratic regression model with $n=20$
 ...
 - `optFederov` requires the candidate list to have more rows than $n$, so we use three replicates of the solvent list
